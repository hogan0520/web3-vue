...
export class NoKuCoinError extends Error {
  public constructor() {
    super('KuCoin not installed')
    this.name = NoKuCoinError.name
    Object.setPrototypeOf(this, NoKuCoinError.prototype)
  }
}
 ...
 */
export interface KuCoinConstructorArgs {
  actions: Actions
  onError?: (error: Error) => void
}

export class KuCoin extends Connector {
  /** {@inheritdoc Connector.provider} */
  public declare provider?: KuCoinProvider
  private eagerConnection?: Promise<void>

  constructor({ actions, onError }: KuCoinConstructorArgs) {
  ...
  private onDisconnect = (error: ProviderRpcError): void => {
    // 1013 indicates that KuCoin is attempting to reestablish the connection
    // https://github.com/MetaMask/providers/releases/tag/v8.0.0
    if (error.code === 1013) {
      console.debug('KuCoin logged connection error 1013: "Try again later"')
      return
    }
  ...
  private async isomorphicInitialize(): Promise<void> {
    ...
    return (this.eagerConnection = Promise.resolve(window.kucoin).then((provider) => {
      if (provider) {
        this.provider = provider as KuCoinProvider
  ...
  protected async _activate(desiredChainIdOrChainParameters?: number | AddEthereumChainParameter): Promise<void> {
    return this.isomorphicInitialize().then(async () => {
      if (!this.provider) throw new NoKuCoinError()
        ...
        .catch((error: ProviderRpcError) => {
          if (error.code === 4902 && typeof desiredChainIdOrChainParameters !== 'number') {
            ...
            return this.provider.request({
              ...
              throw new UnSupportedChainError(desiredChainId)
            })
          }
